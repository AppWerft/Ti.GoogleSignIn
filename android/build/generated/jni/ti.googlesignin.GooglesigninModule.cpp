...
Persistent<FunctionTemplate> GooglesigninModule::proxyTemplate;
jclass GooglesigninModule::javaClass = NULL;

GooglesigninModule::GooglesigninModule(jobject javaObject) : titanium::Proxy(javaObject)
{
}

void GooglesigninModule::bindProxy(Local<Object> exports, Local<Context> context)
{
	Isolate* isolate = context->GetIsolate();

	Local<FunctionTemplate> pt = getProxyTemplate(isolate);
	Local<Function> proxyConstructor = pt->GetFunction(context).ToLocalChecked();
	Local<String> nameSymbol = NEW_SYMBOL(isolate, "Googlesignin"); // use symbol over string for efficiency
	Local<Object> moduleInstance = proxyConstructor->NewInstance(context).ToLocalChecked();
	exports->Set(nameSymbol, moduleInstance);
}

void GooglesigninModule::dispose(Isolate* isolate)
{
	LOGD(TAG, "dispose()");
...
Local<FunctionTemplate> GooglesigninModule::getProxyTemplate(Isolate* isolate)
{
	if (!proxyTemplate.IsEmpty()) {
	...
	javaClass = titanium::JNIUtil::findClass("ti/googlesignin/GooglesigninModule");
	EscapableHandleScope scope(isolate);

	// use symbol over string for efficiency
	Local<String> nameSymbol = NEW_SYMBOL(isolate, "Googlesignin");
...
, javaClass, nameSymbol);
	...
	t->Set(titanium::Proxy::inheritSymbol.Get(isolate),
		FunctionTemplate::New(isolate, titanium::Proxy::inherit<GooglesigninModule>)->GetFunction());
...
void GooglesigninModule::initialize(const FunctionCallbackInfo<Value>& args)
{
	LOGD(TAG, "initialize()");
	Isolate* isolate = args.GetIsolate();
	HandleScope scope(isolate);
	...
	static jmethodID methodID = NULL;
	if (!methodID) {
		methodID = env->GetMethodID(GooglesigninModule::javaClass, "initialize", "(Lorg/appcelerator/kroll/KrollDict;)V");
		if (!methodID) {
