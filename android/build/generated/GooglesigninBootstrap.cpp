...
#define TAG "ti.googlesignin"

using namespace v8;

static Persistent<Object> bindingCache;

static void Googlesignin_getBinding(const FunctionCallbackInfo<Value>& args)
{
	Isolate* isolate = args.GetIsolate();
	EscapableHandleScope scope(isolate);

	if (args.Length() == 0) {
		titanium::JSException::Error(isolate, "Googlesignin.getBinding requires 1 argument: binding");
	...
	titanium::bindings::BindEntry *extBinding = ::GooglesigninBindings::lookupGeneratedInit(
...
static void Googlesignin_init(Local<Object> exports, Local<Context> context)
{
...
static void Googlesignin_dispose(Isolate* isolate)
{
	HandleScope scope(isolate);
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::GooglesigninBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry GooglesigninBinding = {
	"ti.googlesignin",
	Googlesignin_init,
	Googlesignin_dispose
};

// Main module entry point
extern "C" JNIEXPORT void JNICALL
Java_ti_googlesignin_GooglesigninBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("ti.googlesignin", &GooglesigninBinding);
	titanium::KrollBindings::addExternalLookup(&(::GooglesigninBindings::lookupGeneratedInit));
}
